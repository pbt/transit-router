...
export class CanvasOverlay extends google.maps.OverlayView {
  ...
  constructor(handleError: (error: Error) => any) {
    ...
    const scale = bufferMaxWidth * bufferMaxHeight * pixelRatio * pixelRatio <= MAX_BUFFER_SIZE
      ? pixelRatio
      : 1;
  ...
  private refreshBuffer(newData: boolean) {
    ...
    const drawFeature = (feature: Feature, style: DrawingStyle) => {
      ...
      if (geometry.type === 'Point' || geometry.type === 'MultiPoint') {
        ...
        const coords: GeoJSONPoint[] = geometry.type === 'Point'
        ...
        for (const coord of coords) {
          ...
          if (style.image) {
            ...
            if (style.imageDimensions) {
              drawing.onload = () =>
                ctx.drawImage(drawing, x, y, style.imageDimensions[0], style.imageDimensions[1]);
            } else {
              drawing.onload = () => ctx.drawImage(drawing, x, y);
            }
          } else {
            ...
            if (style.text) {
              ...
              ctx.textAlign = style.text.textAlign as any;
              ctx.textBaseline = style.text.textBaseline as any;
        ...
        const lines: GeoJSONPoint[][] = geometry.type === 'LineString'
          ? [geometry.coordinates]
          : geometry.coordinates;
        ctx.beginPath();
        ...
        const polygons: GeoJSONPoint[][][] = geometry.type === 'Polygon'
          ? [geometry.coordinates]
          : geometry.coordinates;
